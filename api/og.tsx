...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  const image = new ImageResponse(
    (
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, color: '#34d399', fontSize: 20, letterSpacing: 6 }}>
          <div style={{ width: 11, height: 11, borderRadius: 11, backgroundColor: '#34d399' }} />
          {T.beacon}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ color: '#34d399', fontSize: 30, letterSpacing: 5, marginTop: 6 }}>
            {T.challenge}
          </div>
        </div>

        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div style={{ color: '#475569', fontSize: 19, letterSpacing: 4 }}>{T.url}</div>
          <div style={{ color: 'rgba(52,211,153,0.9)', fontSize: 88, fontWeight: 700, letterSpacing: -8 }}>{'>_'}</div>
        </div>
