...
import { MAX_RUN_HISTORY, getComparableRuns, type PlayerProgress } from '../services/playerProgress';
...
export default function WeeklyProgress({ language, progress, training }: {
  ...
  const days = getWeeklyProgress(progress.runs, new Date(), progress.practiceDates, getComparableRuns(progress, language));
  const sessions = training.benchmarks.filter(item => item.kind !== 'run' && item.language === language && Number.isFinite(Date.parse(item.completedAt)))
  ...
  return <section className="operator-week" aria-labelledby="operator-week-title">
    <div className="operator-record-section-label"><h3 id="operator-week-title">{text('LAST 7 DAYS', 'ПОСЛЕДНИЕ 7 ДНЕЙ')}</h3>
      <span>{days.filter(day => day.count > 0 || day.practiced).length}/7 {text('ACTIVE DAYS', 'АКТИВНЫХ ДНЕЙ')}</span></div>
    <p>{text(`Daily averages for the current language and matching game conditions, from the last ${MAX_RUN_HISTORY} saved operations. Busy weeks may be incomplete. Dashes mean no recorded operation, not zero speed.`,
      `Средние для текущего языка и одинаковых условий по последним ${MAX_RUN_HISTORY} сохранённым операциям. При частой игре неделя может быть неполной. Прочерк — нет записей, а не нулевая скорость.`)}</p>
    <table>
      <caption className="sr-only">{text('Daily typing speed and accuracy', 'Скорость и точность печати по дням')}</caption>
      <thead><tr><th scope="col">{text('Day', 'День')}</th><th scope="col">{text('Runs', 'Забеги')}</th><th scope="col">{text('Practice', 'Практика')}</th><th scope="col">{text('WPM', 'СЛ/М')}</th><th scope="col">{text('Accuracy', 'Точность')}</th></tr></thead>
      <tbody>{days.map(day => <tr key={day.date} className={day.count ? 'has-signal' : ''}>
        ...
        <td>{day.count || '—'}</td><td>{day.practiced ? '✓' : '—'}</td><td>{day.wpm === null ? '—' : Math.round(day.wpm)}</td><td>{day.accuracy === null ? '—' : `${day.accuracy.toFixed(1)}%`}</td>
      </tr>)}</tbody>
    </table>
