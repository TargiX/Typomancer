...
export function getWeeklyProgress(runs: RunRecord[], now = new Date()) {
  return Array.from({ length: 7 }, (_, index) => {
    ...
    return { date: key, count: items.length,
      wpm: items.length ? items.reduce((sum, run) => sum + run.wpm, 0) / items.length : null,
      accuracy: items.length ? items.reduce((sum, run) => sum + run.accuracy, 0) / items.length : null };
  });
}
